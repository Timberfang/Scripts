function Remove-EmptyDirectory {
	...
	process {
		foreach ($Child in Get-ChildItem $Path -Force -Directory) {
			...
			Remove-Directory $Child.FullName
		}
		$Contents = Get-ChildItem $Path -Force
		if ($null -eq $Contents) {
			Write-Verbose "Removing empty directory '$Path'"
...
function New-File {
	...
	process {
		if (-not (Test-Path $Path)) {
			Write-Verbose "Creating new directory '$Path'"
			New-Item $Path -ItemType Directory
		}
		Write-Verbose "Creating new file '$Name'"
		New-Item $Path -ItemType File -Name $Name | Out-Null
	}
}
...
function New-Directory {
	...
	process {
		Write-Verbose "Creating new directory '$Name'"
		New-Item $Path -ItemType Directory -Name $Name | Out-Null
	}
}
...
function New-Link {
	...
	process {
		...
		if (Test-Path -Path $Path) { Move-Item -Path $Path -Destination $Destination }
		if (!(Test-Path -Path $Destination)) { throw "'$Destination' not found" }
		New-Item -Path $Path -Value $Destination -ItemType Junction
